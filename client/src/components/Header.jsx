       ...
       <div className='flex justify-between items-center max-w-6xl mx-auto p-3'>
        <Link to="/">
            <h1 className='font-bold text-sm sm:text-xl flex flex-wrap '>
                <span className='text-slate-500 '>Seig</span>
        ...
        <ul className='flex gap-4'>
            ...
            <Link to={loggedInUser ? '/profile' : '/sign-in' }>
                {
                    loggedInUser ? (
                        <img className='rounded-full h-7 w-7 object-cover' src={loggedInUser.avatar} atl={loggedInUser.user} />
                    ) : (
